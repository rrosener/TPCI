...
#define P_surface (n_surface * CONST_kB * T)
#define rho_surface (P_surface * (mu * CONST_mH) / (CONST_kB * T))
#define beta (CONST_G * Mp * (mu * CONST_mH) / (Rp * CONST_kB * T))
 ...
 *********************************************************************** */
{
  ...
  double P = P_surface * exp(beta * (Rp / (g_unitLength * r)  - 1));
  if (P < 1e-9) {
    P = 1e-9; //prevent CLOUDY from crashing
  }
  double rho = P * (mu * CONST_mH) / (CONST_kB * T);
 ...
 *********************************************************************** */
{
  ...
  double l_cm = a / (1 + Mp/Ms);
  double r = x1 * g_unitLength;
  double accel = -CONST_G * Mp / (r * r) + CONST_G * Ms / ((a-r) * (a-r)) - CONST_G * (Ms + Mp) / (a * a * a) * (l_cm - r);
  //printf("r=%e, accel=%e\n", r, accel);
