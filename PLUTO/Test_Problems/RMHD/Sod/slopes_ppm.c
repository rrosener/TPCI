...
/* ********************************************************************** */
void STATES (const State_1D *state, int beg, int end, double dt, Grid *grid)
/*
 *
 *
 *  PURPOSE
...
/*  -------------------------------------------
   ...
   CharTracingStep(state, NULL, beg, end, grid);
  #endif
...
/* --------------------------------------------------------------
  ...
  for (i = beg; i <= end; i++){    
    ...
    PRIM_EIGENVECTORS(vc, a2[i], h[i], lambda, L, R);
    #if NVAR != NFLX
  ...
  /* ------------------------------------------------------------------
   ...
   /* -- compute undivided differences and 
     ...
     LpPROJECT(L, dvF[i-1], dwm1);
     LpPROJECT(L, dvF[i  ], dwp1);
     for (k = 0; k < NVAR; k++){
       tau = (dwp1[k] - dwm1[k]); 
       tau = tau*tau;
     ...
     LpPROJECT(L, dvF[i-2], dwm2);
     LpPROJECT(L, dvF[i-1], dwm1);
     LpPROJECT(L, dvF[i  ], dwp1);
     LpPROJECT(L, dvF[i+1], dwp2);
...
/*  -------------------------------------------
  ...
  #if TIME_STEPPING == CHARACTERISTIC_TRACING
   CharTracingStep(state, src, beg, end, grid);
  #endif
  PRIMTOCON (state->vp, state->up, beg, end);
  PRIMTOCON (state->vm, state->um, beg, end);
}
