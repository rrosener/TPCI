...
TEST (ExceptionsInFixtureAreReportedAsHappeningInTheFixture)
{
    ...
    CHECK (std::strstr(reporter.lastFailedMessage, "xception"));
    CHECK (std::strstr(reporter.lastFailedMessage, "fixture"));
    CHECK (std::strstr(reporter.lastFailedMessage, "ThrowingThingie"));
}
...
TEST(FixturesWithThrowingCtorsAreFailures)
{
	...
	CHECK(std::strstr(reporter.lastFailedMessage, "while constructing fixture"));
}
...
TEST(FixturesWithThrowingDtorsAreFailures)
{
	...
	int const failureCount = result.GetFailedTestCount();
	CHECK_EQUAL(1, failureCount);
	CHECK(std::strstr(reporter.lastFailedMessage, "while destroying fixture"));
}
