...
\cdCommand{compile grain} command described on page \pageref{sec:CompileGrains}. In
...
ending in ``\cdFilename{.rfi}'', ``\cdFilename{.mix}'', or
``\cdFilename{.szd}'', resp.). You then type in a single command line, e.g.:
...
\noindent (note that the mixed medium file \cdFilename{fluffy-sil.mix} is not
...
{Example of a mixed medium file for fluffy silicate (\cdFilename{fluffy-sil.mix}).}
\label{mixfile}
\begin{verbatim}
