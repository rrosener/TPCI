 ...
 *  - LIMWINDTAB = 500 - number of points allowed in the tabulated
 *                       velocity
 *  - lgTabulated  - use tablulated velocity                  
 *  - lgTabDepth   - velocity vs. depth or radius            
...
/** limit number of points in tabulated velocity values */
const int LIMWINDTAB = 500;
...
struct Wind {
	...
	/** tabulated wind model */
	bool lgTabulated;
	bool lgTabDepth;
	bool lgTabLinear;

	/** VELTABra is linear/log radius or depth in cm, VEL
	 * heating/cooling erg cm^-3 s^-1 */
	realnum tabrad[LIMWINDTAB];
	realnum tabval[LIMWINDTAB];
