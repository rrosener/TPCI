...
#	endif
	...
	for( long mol=0;mol<mole_global.num_calc;mol++)
	{
		dynamics.molecules[mol] = Upstream_molecules[mol]*scalingDensity();
	}
...
void DynaStartZone(void)
{
	...
	if( (ipUpstream != -1) && (ipUpstream != nOld_zone-1)&& ((Old_depth[ipUpstream+1] - Old_depth[ipUpstream])> SMALLFLOAT) )
	{
		...
		for(mol=0;mol<mole_global.num_calc;mol++)
		{
			...
			if(mole.species[mol].location == NULL && mole_global.list[mol]->parentLabel.empty())
			{
				for(molecule::nAtomsMap::iterator atom= mole_global.list[mol]->nAtom.begin(); 
					 atom != mole_global.list[mol]->nAtom.end(); ++atom)
				{
	...
	for(i=0;i<nzone;++i) 
	{
		...
		for( nelem=ipHYDROGEN; nelem<LIMELM; ++nelem)
		{
			for( ion=0; ion<nelem+2; ++ion )
			{
				...
				dynamics.convergence_error += POW2((double)Oldi_ion/Oldi_density-struc.xIonDense[nelem][ion][i]/scalingZoneDensity(i)) /* *struc.dr[i] */;  
				...
				dynamics.error_scale1 += POW2((double)struc.xIonDense[nelem][ion][i]/scalingZoneDensity(i));			
			}
		}
		for( long ipISO=ipH_LIKE; ipISO<NISO; ++ipISO )
		{
			for( nelem=ipISO; nelem<LIMELM; ++nelem)
			{
				if( dense.lgElmtOn[nelem] )
				{
					for( long level=0; level < iso_sp[ipISO][nelem].numLevels_local; ++level )
					{
						...
						dynamics.convergence_error += POW2(Oldi_iso/Oldi_density-struc.StatesElem[nelem][nelem-ipISO][level][i]/struc.hden[i]) /* *struc.dr[i] */;  

						/* >>chng 02 nov 11, add first error scale estimate from Robin */
						dynamics.error_scale1 += POW2(struc.StatesElem[nelem][nelem-ipISO][level][i]/struc.hden[i]);			
					}
				}
			}
		}
		...
		for( mol=0; mol < mole_global.num_calc; mol++)
		{
			...
			dynamics.convergence_error += POW2((double)Oldi_mol/Oldi_density-struc.molecules[mol][i]/scalingZoneDensity(i)) /* *struc.dr[i] */;  

			/* >>chng 02 nov 11, add first error scale estimate from Robin
			 * used to normalize the above convergence_error */
			dynamics.error_scale1 += POW2((double)struc.molecules[mol][i]/scalingZoneDensity(i));			
		}
	}
...
STATIC void DynaSaveLast(void)
{
	...
	for( i=0; i<nzone; ++i )
	{
		...
		for(mol=0;mol<mole_global.num_calc;mol++)
		{
			Old_molecules[i][mol] = struc.molecules[mol][i];
		}
		...
		for( long ipISO=ipH_LIKE; ipISO<NISO; ++ipISO )
		{
			for( nelem=ipISO; nelem<LIMELM; ++nelem)
			{
				if( dense.lgElmtOn[nelem] )
				{
					for( long level=0; level < iso_sp[ipISO][nelem].numLevels_max; ++level )
					{ 
						Old_StatesElem[i][nelem][nelem-ipISO][level] = struc.StatesElem[nelem][nelem-ipISO][level][i];
...
realnum DynaFlux(double depth)
{
	...
	if( wind.lgTabulated )
	{
		/* interpolate velocity on tabulated values */
		flux = (realnum)interpol_tabulated( 
					radius.Radius, radius.depth,
					wind.lgTabDepth, wind.lgTabLinear,
					wind.tabrad, wind.tabval,
					wind.nvals );
...
void DynaCreateArrays( void )
{
	...
	{
		Upstream_molecules = (double*)MALLOC((size_t)mole_global.num_calc*sizeof(double) );
		
		dynamics.molecules = (double*)MALLOC((size_t)mole_global.num_calc*sizeof(double) );
	}
	else
	{
		Upstream_molecules = dynamics.molecules = NULL;
	}
	...
	Old_density = ((realnum*)MALLOC( (size_t)(struc.nzlim)*sizeof(realnum )));
...
#	endif
	...
	if( p.nMatch("TABL") )
	{
		...
		ParseTabulated( p, &wind.lgTabDepth, &wind.lgTabLinear, wind.tabrad, wind.tabval, &wind.nvals );

		/* set initial velocity and static + ballistic = false */
		wind.windv0 = wind.tabval[0];
		wind.windv = wind.windv0;
		wind.setDefault();
